...
### Installing Eclipse & JDK for Macs
1. Go to the **[Eclipse downloads](https://www.eclipse.org/downloads/)**, and under **Get Eclipse SimRel 2018-09**, click **Download Packages**.
...
### Installing Eclipse & JDK for Linux
1. Go to the **[Eclipse downloads](https://www.eclipse.org/downloads/)**, and under **Get Eclipse SimRel 2018-09**, click **Download Packages**.
...
GitHub is a really popular online platform for **working on coding projects collaboratively** with other people anywhere, all over the world. You can find a lot of different projects there that you can contribute to, and get help for any of your own projects there from other people as well. One of the most crucial features of GitHub is **version control**. This means that if you commit code for something, overwriting code you wrote before that you thought was incorrect, but later realize the previous version was better than your most recent changes, you can **easily revert your code** to those changes because **GitHub tracks those changes you committed**. It also allows contributors to **merge changes they did without accidentally overwriting** changes from other contributors. Think of GitHub as saving multiple separate drafts of an essay, mixed with the collaborative nature of Google Documents, but with code.
...
### Setting Up Your GitHub Account for the Class and Obtaining a Copy of the Class Repository (both locally and on your account online)
1. While logged into your account, go to the [class repository](https://github.com/alyeffy/AP-CS-A_2018-2019).
2. In the top right corner, click the **Fork** button to have a copy of all the code and other files in the repository onto your account. You now have a **copy** of the class repository that contains everything it has **from the time you copied it**.
3. Open **Eclipse**, go to `File > Import > Git > Projects from Git`, and click **Clone URL**.
4. Go to your account page with copy of the class repository. It should be at `https://github.com/your_username/AP-CS-A_2018-2019`.
...
7. When it asks you to select a wizard for importing projects, select **Import as general project**. You now have the repository accessible via Eclipse.
8. You now need to make sure that when you make, save, and commit changes to your code in your repository copy, it will get pushed to your online copy, so that myself and other students can see the changes.
9. Open your command line, and **navigate to your local repository copy** (the directory you cloned it to above) **within the command line** using the these commands.
```bash
...
```
e.g. if you are in the `/Users/your_username` directory (use pwd to check), and the local repository is in the `/Users/your_username/git/AP-CS-A_2018-2019` directory, then you need to type in and enter:
```bash
...
### Updating Your Local Repository with the Original One Without Overwriting Your Code - Using the Command Line
...
3. Enter `git fetch upstream` to get all the changes from the Original master branch, which will be placed into the `upstream/master` branch in your repository.
...
3. Now, if you are working on an **assignment**, **first obtain any code you need** from the original repository to the **master branch** of your local repository. Then switch to the **assignments branch** by entering `git checkout assignments`, then enter `git pull origin master` to obtain any updates to the master branch of your local repository for your **assignments branch** as well. This way, you will have a local copy of original unedited assignment code files in the **master branch**, and the assignment code files with your changes in the **assignments branch**.
